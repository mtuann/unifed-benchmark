...
def handle_stu(matr):
    return np.array(np.hstack((matr[:,1:8],matr[:,9:])), dtype = np.float32), np.array(matr[:,8],dtype=np.float32)
...
def read_data(train_data_dir, test_data_dir):
    ...
    for f in train_files:
        ...
        if not (file_path[-9] == 'g') :
            train_data['host']['x'],train_data['host']['y'] = handle_stu(my_numpy)
        else:
            train_data['guest']['x'],train_data['guest']['y'] = handle_stu(my_numpy)
    ...
    for f in test_files:
        file_path = os.path.join(test_data_dir, f)
        my_numpy = np.array(pd.read_csv(file_path))
        test_data['host']['x'],test_data['host']['y'] = test_data['guest']['x'],test_data['guest']['y'] = handle_stu(my_numpy)

    # print('------------------------')
    # print(train_data['host']['x'][:10])
    # print('******************')
    return clients, groups, train_data, test_data
...
def load_partition_data_student_horizontal(batch_size,
                              train_path="../csv_data/student_horizontal_train/",
                              test_path="../csv_data/student_horizontal_test/"):
